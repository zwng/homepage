...
function initQRCode() {
    ...
    function generateQRCode() {
        ...
        if (typeof QRCode === 'undefined') {
            console.error('QRCode库未加载，显示备用方案');
            showFallbackText(currentURL);
            return;
        }
        ...
        try {
            ...
            QRCode.toCanvas(qrCanvas, currentURL, {
                ...
                if (error) {
                    ...
                    // 尝试备用方案
                    generateQRCodeWithQrcodeLib(currentURL);
                } else {
                    console.log('二维码生成成功');
                }
            });
        } catch (error) {
            console.error('QRCode生成异常:', error);
            showFallbackText(currentURL);
        }
    }
    ...
    // 显示备用文本
    function showFallbackText(url) {
        qrCanvas.style.display = 'none';
        const fallbackText = document.createElement('div');
        fallbackText.className = 'qr-fallback';
        fallbackText.innerHTML = `
      <div style="text-align: center; padding: 20px;">
        <p style="color: #7f8c8d; margin-bottom: 10px;">请复制以下链接到手机浏览器：</p>
        <p style="background: #f8f9fa; padding: 10px; border-radius: 5px; word-break: break-all; font-family: monospace; border: 1px solid #e9ecef;">
          ${url}
        </p>
        <button onclick="copyToClipboard('${url}')" 
                style="margin-top: 10px; padding: 8px 16px; background: #3498db; color: white; border: none; border-radius: 5px; cursor: pointer;">
          复制链接
        </button>
        <div style="margin-top: 15px; padding: 10px; background: #e8f4fd; border-radius: 5px; border-left: 4px solid #3498db;">
          <p style="margin: 0; font-size: 14px; color: #2c3e50;">
            <i class="fas fa-info-circle"></i> 
            提示：如果二维码无法显示，请直接复制链接到手机浏览器打开
          </p>
        </div>
      </div>
    `;
        qrCanvas.parentNode.appendChild(fallbackText);
    }
    ...
    function showCopySuccess() {
      ...
      <div style="position: fixed; top: 20px; right: 20px; background: #27ae60; color: white; padding: 10px 20px; border-radius: 5px; z-index: 10000; animation: slideIn 0.3s ease;">
    ...
    function addShareFeature() {
        ...
        qrInfo.appendChild(shareButton);
    }
